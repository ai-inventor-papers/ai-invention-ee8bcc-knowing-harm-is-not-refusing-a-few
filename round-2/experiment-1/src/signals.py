...
def interpolation_sharpness(
    ...
    na = len(ben_encs)
    grid = layer_grid(L)
    # Patch vectors must match the model's working dtype: gemma-3-4b-it runs in
    # bf16 (fp16 overflows its residual stream), so hardcoding float16 here
    # would raise a dtype-mismatch inside the forward-pre hook.
    _patch_dtype = next(iter(h.model.parameters())).dtype
    ...
    for lay in grid:
        ...
        for i in range(na):
            ...
            for a in alphas:
                ...
                rows.append({
                    "enc": ben_encs[i],
                    "z": torch.as_tensor(z, dtype=_patch_dtype).to(h.model.device),
                })
...
def spectral_signature(model_id: str, L: int) -> dict[str, Any]:
    ...
    def _svd_stats(t: np.ndarray) -> tuple[float, float]:
        ...
        # t @ t^T or t^T @ t).  Iteration-1 did the fp64 CPU eigvalsh, which the
        # 6-core container takes ~90-150 s per 2560x2560 Gram (8 Grams per model
        # -> tens of minutes on Gemma-3-4B).  GPU fp32 torch.linalg.eigvalsh is
        # exact-spectrum-equivalent (same Gram, fp32) and takes milliseconds; the
        # fp64 CPU path is kept as an identical-results fallback.
        a, b = t.shape
        try:
            import torch as _torch
            if not _torch.cuda.is_available():
                raise RuntimeError("no cuda available")
            tt = _torch.as_tensor(t, dtype=_torch.float32, device="cuda")
            gram = tt @ tt.T if a <= b else tt.T @ tt
            w = _torch.linalg.eigvalsh(gram).to("cpu").numpy()
            del tt, gram
    ...
    # Weight-key prefixes by architecture: plain causal LMs store the decoder
    # under "model.layers."; multimodal wrappers (Gemma3ForConditionalGeneration)
    # prefix the text decoder with "language_model." / "model.language_model.".
    _prefixes = ("", "language_model.", "model.language_model.", "model.model.")
    dw_names = {f"{p}model.layers.{i}.mlp.down_proj.weight"
                for p in _prefixes for i in late}
    uw_names = {f"{p}model.layers.{i}.mlp.up_proj.weight"
                for p in _prefixes for i in late}
    for f in files:
        try:
